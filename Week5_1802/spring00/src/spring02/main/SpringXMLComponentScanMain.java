package spring02.main;

import org.springframework.context.support.ClassPathXmlApplicationContext;
import spring01.model.GenericInfoService;
import spring02.model.EmailAddress;

public class SpringXMLComponentScanMain {

	public static void main(String[] args) {

		System.out.println("Spring using XML Configuration with Component Scanning\n");
		...
		/*
		 * "emailServiceBean" with constructor injection with literal value injection
		 * (using @Value) and object reference injection (using @Autowired @Qualifier)
		 * and
		 * 
		 * @PostConstruct (equivalent to init-method in xml config) and
		 * 
		 * @PreDestroy (equivalent to destroy-method in xml config) See:
		 * spring02.model.EmailInfoService
		 */
